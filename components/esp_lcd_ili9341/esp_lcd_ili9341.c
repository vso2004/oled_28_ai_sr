                                    ...
                                    const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    int rrr = panel_dev_config->bits_per_pixel;
    switch (rrr)
    {
...
const ili9341_lcd_init_cmd_t vendor_specific_init_default1[] = {
    //  {cmd, { data }, data_size, delay_ms}
    /* Power control B */
    {0xCF, (uint8_t []){0x00, 0xC1, 0x30}, 3, 0},
    /* Power on sequence control */
...
const ili9341_lcd_init_cmd_t vendor_specific_init_default2[] = {
...
static esp_err_t panel_ili9341_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]){ili9341->madctl_val}, 1), TAG, "send command failed");
    return ESP_OK;
}
